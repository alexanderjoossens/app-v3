...
function insertTest(vector1test, vector2test) {
  ...
  execSql("INSERT INTO vectors2(vector1) VALUES (?);",[vector1test])
  .then(function (result) {
    console.log("Inserted " + vector1test);
    return execSql("SELECT id, vector1 from vectors2 where vector1 = ?;", vector1test);
  })
  .then((result) => {
    let { id, vector1test } = result[0];
    console.log("Result: " + id + " " + vector1test);
    return execSql("INSERT INTO vectors2 (vector2,sel_pl2) VALUES (?,?);",
                      [vector2test, 'test3']);
  })
  .then((result) => {
...
//   let sql = "CREATE TABLE vectors(id int AUTO_INCREMENT, code VARCHAR(225),vector1 VARCHAR(225),vector2 VARCHAR(225),PRIMARY KEY(id))";
//   db.query(sql,(err,result)=>{
//     if (err) throw err;
//   console.log(result);
...
export async function addPlaylistSelectionToDB(code,selectedPlaylists) {
    ...
    if (currentUser==1) {
        ...
        sql = `UPDATE vectors2 SET sel_pl1='${selectedPlaylistsReformat}' WHERE code='${code}'`;
    }
    else {
        post = {code:code, sel_pl2:selectedPlaylists}
        sql = `UPDATE vectors2 SET sel_pl2='${selectedPlaylistsReformat}' WHERE code='${code}'`;
    }
...
export async function isCodeInDB(codeToCheck, updateDBPlaylistInfo=false) {
  ...
  let sql2 = "SELECT id, vector1, vector2 FROM vectors2 WHERE code = "+db.escape(codeToCheck);
...
export async function insertVector1andRecSongDataA(codeToInsert, vectorToInsert, recommended_tracks,recommended_tracks_urls,recommended_tracks_names,recommended_tracks_artists) {
  ...
  let sql = `INSERT INTO vectors2 SET ?`
  let query = db.query(sql,post,(err,result)=>{
    if (err) throw err;
  });
...
export async function insertVector2andRecSongDataB(codeToInsert, vectorToInsert, id, recommended_tracks,recommended_tracks_urls,recommended_tracks_names,recommended_tracks_artists, selected_track_ids, selected_artist_ids) {
  let sql = `UPDATE vectors2 SET vector2='${vectorToInsert}' WHERE id='${id}'`;
...
export async function addSelectedTrackIdsToDB(user, code, selected_track_ids) {
  let sqlTr = ''
  if (user == 'a') {
    sqlTr = `UPDATE vectors2 SET tracksA='${JSON.stringify(selected_track_ids.toString())}' WHERE code='${code}'`
  }
  else {
    sqlTr = `UPDATE vectors2 SET tracksB='${JSON.stringify(selected_track_ids.toString())}' WHERE code='${code}'`
  }
...
export async function addSelectedArtistIdsToDB(user, code, selected_artist_ids) {
  let sqlAr = ''
  if (user == 'a') {
    sqlAr = `UPDATE vectors2 SET artistsA='${JSON.stringify(selected_artist_ids.toString())}' WHERE code='${code}'`
  }
  else {
    sqlAr = `UPDATE vectors2 SET artistsB='${JSON.stringify(selected_artist_ids.toString())}' WHERE code='${code}'`
  }
...
export async function insertRecSongDataToDB(user,code,recommended_tracks, recommended_tracks_urls, recommended_tracks_names, recommended_tracks_artists) {
  ...
  if (user == 'a') {
    ...
    // console.log('JSON.stringify(recTracksObj): ', JSON.stringify(recTracksObj))
    //sql = `UPDATE vectors2 SET recTracksA= {"test":"true"} WHERE code='${code}'`;
    sql = `UPDATE vectors2 SET recTracksA='${JSON.stringify(recTracksObj).replaceAll(/\\/g, "\\\\")}' WHERE code='${code}'`
    // sql = `UPDATE vectors2 SET recTracksA='${JSON.stringify(recommended_tracks)}' WHERE code='${code}'`;

    let sql_no_single_quotes = sql.slice(32,sql.length-21)
    if (sql_no_single_quotes.includes("'")) {
      sql = sql.slice(0,32)+sql_no_single_quotes.replaceAll("'","&quot;") + sql.slice(sql.length-21,sql.length)
    }
    // console.log('sql: ',sql)
    let query = db.query(sql,(err,result)=>{
      if (err) throw err;
    ...
    sql = `UPDATE vectors2 SET recTracksB='${JSON.stringify(recTracksObj).replace(/\\/g, "\\\\")}' WHERE code='${code}'`
    let sql_no_single_quotes = sql.slice(32,sql.length-21)
    if (sql_no_single_quotes.includes("'")) {
      sql = sql.slice(0,32)+sql_no_single_quotes.replaceAll("'",'&quot;') + sql.slice(sql.length-21,sql.length)
    }
  ...
  //   sql = `UPDATE vectors2 SET recSongsB='${recSongData}' WHERE code='${code}'`;
  // }
  // console.log('sql: ',sql)
  // let query = db.query(sql,(err,result)=>{
  //   if (err) throw err;
